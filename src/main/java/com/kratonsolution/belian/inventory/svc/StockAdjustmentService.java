...
public class StockAdjustmentService
{
	...
	@Secured("ROLE_STOCK_ADJUSTMENT_READ")
	public StockAdjustment getOne(String id)
	{
	...
	public void add(StockAdjustment stock)
	{
		...
		for(StockAdjustmentItem item:stock.getItems())
		{
			...
			if(item.getExpired() != null)
				inv = itemRepository.getOne(item.getProduct().getId(), stock.getFacility().getId(),item.getContainer().getId(),item.getExpired());
			else
				inv = itemRepository.getOne(item.getProduct().getId(), stock.getFacility().getId(),item.getContainer().getId());
	...
	{
		StockAdjustment stock = repository.getOne(id);
		if(stock != null)
		{
			for(StockAdjustmentItem item:stock.getItems())
			{
