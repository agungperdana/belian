...
public class PractitionerGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new PractitionerFormContent());
			}
		});
	...
	{
		final PractitionerModel model = new PractitionerModel(utils.getRowPerPage());
		...
		grid.setModel(model);
		grid.setRowRenderer(new PractitionerRenderer());
		grid.setPagingPosition("both");
		grid.setMold("paging");
		grid.setPageSize(utils.getRowPerPage());
		grid.appendChild(new Columns());
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.practitioner.grid.column.start"),null,"100px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.practitioner.grid.column.code"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.practitioner.grid.column.name"),null,"200px"));
				...
				for(Component com:target.getRows().getChildren())
				{
					com.addEventListener(Events.ON_CLICK,new EventListener<Event>()
					{
						...
						public void onEvent(Event ev) throws Exception
						{
							...
							Flow.next(getParent(), new PractitionerEditContent(row));
						}
					});
				}
			}
