...
public class ProductGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(task.getFocused(), new ProductDashboard(new Product()));
			}
		});
	...
	protected void initGrid()
	{
		...
		filter.addEventListener(Events.ON_CHANGING, new EventListener<InputEvent>()
		{
			...
			{
				grid.setModel(new ProductModel(utils.getRowPerPage(), input.getValue()));
			}
		});
		...
		grid.getColumns().appendChild(new Column(lang.get("inventory.product.grid.column.supportend"),null,"100px"));
				...
				for(Component com:target.getRows().getChildren())
				{
					com.addEventListener(Events.ON_CLICK,new EventListener<Event>()
					{
						...
						public void onEvent(Event ev) throws Exception
						{
							...
							Flow.next(getParent(), new ProductDashboard(service.getOne(RowUtils.id(row))));
						}
					});
				}
			}
