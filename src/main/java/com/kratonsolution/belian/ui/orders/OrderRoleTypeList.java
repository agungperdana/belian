 ...
 */
public class OrderRoleTypeList extends Listbox
{
	private SessionUtils utils = Springs.get(SessionUtils.class);
	
	private Map<String,OrderRoleType> maps = new HashMap<String, OrderRoleType>();
	
	public OrderRoleTypeList(boolean fullspan)
	{
		this(fullspan,null);
	}
	
	public OrderRoleTypeList(boolean fullspan,OrderRoleType con)
	{
		if(fullspan)
			setWidth("100%");
		else
		...
		for(OrderRoleType type:OrderRoleType.values())
		{
	...
	public void setOrderRoleType(OrderRoleType type)
	{
		if(type != null)
		{
			getItems().clear();
