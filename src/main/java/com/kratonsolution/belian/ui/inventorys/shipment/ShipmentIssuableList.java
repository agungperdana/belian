 ...
 */
public class ShipmentIssuableList extends AbstractList<Shipment> implements Observer
{	
	private ShipmentService service = Springs.get(ShipmentService.class);
	
	public ShipmentIssuableList(boolean fullspan)
	{
		this(fullspan,null);
	}
	
	public ShipmentIssuableList(boolean fullspan,Shipment con)
	{
		if(fullspan)
			setWidth("100%");
		else
	...
	public void valueChange(IDValueRef value)
	{
		if(value != null && !Strings.isNullOrEmpty(value.getId()))
		{
			...
			{
				if(shipment.isEditable() && shipment.getShipToParty().getId().equals(value.getId()))
				{
