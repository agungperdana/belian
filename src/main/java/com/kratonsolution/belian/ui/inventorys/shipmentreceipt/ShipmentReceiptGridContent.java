 ...
 */
public class ShipmentReceiptGridContent extends GridContent
{
	private ShipmentReceiptService service = Springs.get(ShipmentReceiptService.class);
	
	public ShipmentReceiptGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new ShipmentReceiptForm());
			}
		});
	...
	{
		final ShipmentReceiptModel model = new ShipmentReceiptModel(utils.getRowPerPage());
		...
		grid.setModel(model);
		grid.setRowRenderer(new ShipmentReceiptRowRenderer());
		grid.setPagingPosition("both");
		grid.setMold("paging");
		grid.setPageSize(utils.getRowPerPage());
		grid.appendChild(new Columns());
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column(lang.get("shipmentreceipt.grid.column.number"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("shipmentreceipt.grid.column.date"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("shipmentreceipt.grid.column.organization"),null,"200px"));
			...
			public void onEvent(Event event) throws Exception
			{
				...
				for(Component com:target.getRows().getChildren())
				{
					com.addEventListener(Events.ON_CLICK,new EventListener<Event>()
					{
						...
						{
							Flow.next(getParent(), new ShipmentReceiptEditForm((Row)ev.getTarget()));
						}
					});
				}
			}
