 ...
 */
public class SalesInvoiceItemForm extends AbstractForm
{	
	...
	public SalesInvoiceItemForm(SalesInvoice invoice)
	{
		super();
	...
	public void initToolbar()
	{
		toolbar.getCancel().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			{
				Flow.next(getParent(), new SalesInvoiceEditContent(RowUtils.shield(invoice.getId())));
			}
		});
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				SalesInvoice out = service.getOne(invoice.getId());
				if(out != null)
				{
						...
						for(Component com:shipments.getRows().getChildren())
						{
							...
							if(RowUtils.isChecked(row))
							{
								...
								if(shipmentItem != null)
								{
									...
									billing.setItem(invoiceItem);
									billing.setShipment(shipmentItem.getShipment().toRef());
									billing.setShipmentItem(shipmentItem.toRef());
					...
					else if(invoiceItem.getType().equals(InvoiceItemType.INVOICE_WORK_EFFORT))
					{
						for(Component com:efforts.getRows().getChildren())
						{
							...
							{
								WorkEffort effort = workEffortService.getOne(RowUtils.id(rw));
								if(effort != null)
								{
	...
	private void initDetails()
	{
		...
		tabbox.getTabs().appendChild(new Tab(lang.get("invoices.grid.column.orderitems")));
		tabbox.getTabs().appendChild(new Tab(lang.get("invoices.grid.column.shipmentitems")));
		tabbox.getTabpanels().appendChild(new Tabpanel());
		tabbox.getTabpanels().appendChild(new Tabpanel());
		tabbox.getTabpanels().getFirstChild().appendChild(orders);
		...
		orders.getColumns().appendChild(new Column(lang.get("invoices.grid.column.number"),null,"85px"));
		...
		shipments.getColumns().appendChild(new Column(lang.get("invoices.grid.column.quantity"),null,"85px"));
	...
	private void initEfforts()
	{
		...
		for(WorkEffort effort:workEffortService.findAllUninvoiced())
		{
			...
			{
				OrderItem orderItem = orderItemService.getOne(fulfillment.getOrderItem().getId());
	...
	private void initProduct()
	{
		if(product.getDomain() != null)
		{
			...
			orders.getRows().getChildren().clear();
			shipments.getRows().getChildren().clear();

			/**
			 * populate data from sales order
			 */
			for(SalesOrder so:salesOrderService.findAllInvoiceable(invoice.getBilledToParty().getId()))
			{
				if(so.isOpen())
				{
					for(OrderItem orderItem:so.getItems())
					{
						if(!orderItem.isInvoiced() && orderItem.getType().equals(OrderItemType.PRODUCT) && product.getDomain().getId().equals(orderItem.getProduct().getId()))
						{
							Checkbox check = Components.checkbox(false);
							check.addEventListener(Events.ON_CHECK, new EventListener<Event>()
							{
							...
							row.appendChild(new Label(orderItem.getOrder().getNumber()));
							row.appendChild(Components.label(orderItem.getQuantity().subtract(orderItem.getInvoiced())));
							row.appendChild(Components.label(orderItem.getUntitPrice()));
							row.appendChild(new Label(orderItem.getId()));

							orders.getRows().appendChild(row);
						}
					}
				}
			}
			...
			/**
			 * populate data from shipment
			 */
			for(Shipment shipment:shipmentService.forSalesInvoice(invoice.getBilledToParty().getId()))
			{
						...
						for(ShipmentOrder order:item.getOrders())
						{
							if(!order.isInvoiced())
							{
								...
								if(orderItem != null && product.getDomain().getId().equals(item.getProduct().getId()))
								{
									...
									Row rw = new Row();
									rw.appendChild(check);
									rw.appendChild(new Label(DateTimes.format(orderItem.getOrder().getEntryDate())));
									rw.appendChild(new Label(orderItem.getOrder().getNumber()));
									rw.appendChild(Components.label(order.getQuantity()));
									rw.appendChild(Components.label(order.getUnitPrice()));
									rw.appendChild(Components.label(order.getUnitPrice().multiply(order.getQuantity())));
									rw.appendChild(new Label(order.getId()));

									shipments.getRows().appendChild(rw);
								}
							}
						}
					}
		...
		if(itemTypeList.getDomain().equals(InvoiceItemType.INVOICE_PRODUCT) || itemTypeList.getDomain().equals(InvoiceItemType.INVOICE_PRODUCT_FEATURE))
		{
			for(Component com:orders.getRows().getChildren())
			{
				...
				if(RowUtils.isChecked(row))
				{
					...
					total = total.add(RowUtils.decimal(row, 3).multiply(RowUtils.decimal(row, 4)));
				}
			}
			...
			for(Component com:shipments.getRows().getChildren())
			{
				...
				{
					quan = quan.add(RowUtils.decimal(row, 3));
					total = total.add(RowUtils.decimal(row, 3).multiply(RowUtils.decimal(row, 5)));
				}
			}
		}
