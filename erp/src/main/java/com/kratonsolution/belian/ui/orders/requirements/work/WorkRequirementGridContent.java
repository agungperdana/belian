 ...
 */
public class WorkRequirementGridContent extends GridContent
{
	private WorkRequirementService service = Springs.get(WorkRequirementService.class);
	
	public WorkRequirementGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new WorkRequirementFormContent());
			}
		});
	...
	{
		final WorkRequirementModel model = new WorkRequirementModel(utils.getRowPerPage());
		...
		grid.setModel(model);
		grid.setRowRenderer(new WorkRequirementRowRenderer());
				...
				for(Component com:target.getRows().getChildren())
				{
					com.addEventListener(Events.ON_CLICK,new EventListener<Event>()
					{
						...
						public void onEvent(Event ev) throws Exception
						{
							...
							Flow.next(getParent(), new WorkRequirementEditContent(row));
						}
					});
				}
			}
