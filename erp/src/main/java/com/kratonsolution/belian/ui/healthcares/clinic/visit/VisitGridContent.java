 ...
 */
public class VisitGridContent extends GridContent
{
	private VisitService service = Springs.get(VisitService.class);
	
	public VisitGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new VisitFormContent());
			}
		});
	...
	{
		final VisitModel model = new VisitModel(utils.getRowPerPage());
		...
		grid.setModel(model);
		grid.setRowRenderer(new VisitRenderer());
		grid.setPagingPosition("both");
		grid.setMold("paging");
		grid.setPageSize(utils.getRowPerPage());
		grid.appendChild(new Columns());
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.visit.grid.column.date"),null,"85px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.visit.grid.column.patient"),null,"175px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.visit.grid.column.doctor"),null,"175px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.visit.grid.column.reason"),null,"200px"));
		grid.getColumns().appendChild(new Column(lang.get("clinic.visit.grid.column.note"),null,"150px"));
				...
				for(Component com:target.getRows().getChildren())
				{
					com.addEventListener(Events.ON_CLICK,new EventListener<Event>()
					{
						...
						public void onEvent(Event ev) throws Exception
						{
							...
							Flow.next(getParent(), new VisitEditContent(row));
						}
					});
				}
			}
