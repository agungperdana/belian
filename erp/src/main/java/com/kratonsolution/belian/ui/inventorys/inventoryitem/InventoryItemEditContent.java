 ...
 */
public class InventoryItemEditContent extends AbstractForm
{	
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				InventoryItem inv = service.getOne(RowUtils.id(row));
				if(inv != null)
				{
	...
	{
		InventoryItem out = service.getOne(RowUtils.id(row));
		if(out != null)
		{
			organizations.setDomainAsRef(out.getOrganization());
			products.setDomainAsRef(out.getProduct());
			facilitys.setDomainAsRef(out.getFacility());
			containers.setDomainAsRef(out.getContainer());
			onhand.setValue(out.getOnhand());
			expired.setValue(out.getExpiredDate());
			serials.setValue(out.getSerialNumber());
		}
