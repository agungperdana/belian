 ...
 */
public class SalesOrderItemForm extends BForm
{	
	...
	public SalesOrderItemForm(SalesOrderItem item)
	{
		super();
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				if(priceBox.getNominal() == null || priceBox.getNominal().getValue() == null || priceBox.getNominal().getValue().compareTo(BigDecimal.ZERO) <= 0)
					throw new WrongValueException(priceBox,lang.get("message.field.empty"));
				...
				if(item.getOrder() != null)
				{
					...
					item.setUntitPrice(priceBox.getNominal().getValue());
	...
	public void initForm()
	{
		if(item != null)
		{
			priceBox.setCustomer(item.getOrder().getBillToParty());
		}
		...
		priceBox.addObserver(new Observer()
		{
			@Override
		...
		featureList.addObserver(priceBox);
		featureList.addObserver(new Observer()
		{
			...
			public void valueChange(IDValueRef value)
			{
				if(value != null && !Strings.isNullOrEmpty(value.getId()))
				{
					...
					for(ProductRequirement requirement:productRequirementService.findAllFeatureActive(item.getOrder().getPartyPlacingOrder().getId(),value.getId(),RequirementType.CUSTOMER_REQUIREMENT))
					{
		...
		productBox.addObserver(priceBox);
		productBox.addObserver(new Observer()
		{
			...
			public void valueChange(IDValueRef value)
			{
				if(value != null && !Strings.isNullOrEmpty(value.getId()))
				{
					...
					for(ProductRequirement requirement:productRequirementService.findAllProductActive(item.getOrder().getPartyPlacingOrder().getId(),value.getId(),RequirementType.CUSTOMER_REQUIREMENT))
					{
						Checkbox checkbox = Components.checkbox(false);
						checkbox.addEventListener(Events.ON_CHECK, new EventListener<Event>()
						{
	...
	private void calculate()
	{
		...
		total.setText(Numbers.format(quantity.getValue().multiply(priceBox.getNominal().getValue())));
	}
}
