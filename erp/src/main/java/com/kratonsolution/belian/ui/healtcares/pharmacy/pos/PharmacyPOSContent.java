...
public class PharmacyPOSContent extends POSOrder
{
	...
	private void initLeft()
	{
		...
		prescrip.setOrient("vertical");
		prescrip.setWidth("110px");
		prescrip.addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			@Override
			public void onEvent(Event arg0) throws Exception
			{
				PharmacyPOSPrescription prescription = new PharmacyPOSPrescription();
				prescription.getOk().addEventListener(Events.ON_CLICK, new EventListener<Event>()
				{
					@Override
					public void onEvent(Event arg0) throws Exception
					{
						if(prescription.getCustomer().getValue() == null)
							throw new WrongValueException(prescription.getCustomer(),lang.get("message.field.empty"));

						if(prescription.getDoctor().getValue() == null)
							throw new WrongValueException(prescription.getDoctor(),lang.get("message.field.empty"));

						saleType = SaleType.STANDARD_REFERENCE_SALES;
						
						doctorDisplay.setValue(prescription.getDoctor().getText());
						customerDisplay.setValue(prescription.getCustomer().getText());
		...
		for(OrderAdjustmentType type:OrderAdjustmentType.values())
		{
			if(!type.equals(OrderAdjustmentType.FEE))
			{
				...
				button.addEventListener(Events.ON_CLICK, new EventListener<Event>()
				{
					...
					public void onEvent(Event arg0) throws Exception
					{
						PharmacyPOSAdjustment poa = new PharmacyPOSAdjustment();
			...
			if(product.getName().startsWith(tab.getLabel()))
			{
				...
				listitem.addEventListener(Events.ON_CLICK, new EventListener<Event>()
				{
					...
					public void onEvent(Event arg0) throws Exception
					{
						...
						Product fresh = productService.getOne(product.getId());
	...
	public void calculateResult()
	{
		...
		for(Component com:items.getRows().getChildren())
		{
			...
			Product product = productService.getOne(RowUtils.id(row));
			if(product != null)
	...
	private String createOrder(PaymentMethodInfo info)
	{
		 ...
		 * Payer Information
		 */
		Party payer = partyService.getOne(customer.getId());
		if(payer != null)
		{
		 ...
		 */
		Party receiver = partyService.getOne(shipto.getId());
		if(receiver != null)
		{
		...
		for(Component com:items.getRows().getChildren())
		{
			...
			Product product = productService.getOne(RowUtils.id(rw));
			if(product != null)
			{
