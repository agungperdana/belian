 ...
 */
public class WorkEffortFormContent extends AbstractForm
{	
	...
	private SalesOrderService salesOrderService = Springs.get(SalesOrderService.class);
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				effort.setNumber(generator.generate(effort.getCreationDate(),effort.getOwner().getId(),Code.WEF));
				...
				for(Component com:assignments.getRows().getChildren())
				{
					...
					assignment.setStart(RowUtils.sql(row, 1));
					assignment.setEnd(RowUtils.sql(row, 2));
				...
				for(Component com:statuses.getRows().getChildren())
				{
					...
					WorkEffortStatus status = new WorkEffortStatus();
					status.setEffort(effort);
					status.setEnd(RowUtils.sql(row, 2));
					status.setStart(RowUtils.sql(row, 1));
				...
				for(Component com:rates.getRows().getChildren())
				{
					...
					WorkEffortPartyRate rate = new WorkEffortPartyRate();
					rate.setEffort(effort);
					rate.setStart(RowUtils.sql(row, 1));
					rate.setEnd(RowUtils.sql(row, 2));
					rate.setParty(box.getDomainAsRef());
					rate.setType(rateType.getDomain());
					rate.setRate(RowUtils.decimal(row, 4));
					rate.setCommend(RowUtils.string(row, 6));
					
					effort.getRates().add(rate);
				}
				
				for(Component com:requirementFulfillments.getRows().getChildren())
				{
					Row row = (Row)com;
					
					if(RowUtils.isChecked(row))
					{
						WorkRequirement requirement = workRequirementService.getOne(RowUtils.id(row));
						if(requirement != null)
						{
							WorkRequirementFulfillment fulfillment = new WorkRequirementFulfillment();
							fulfillment.setEffort(effort);
							
							effort.getFulfillments().add(fulfillment);
						}
					}
				}
				
				for(Component com:orderFulfillments.getRows().getChildren())
				{
					Row row = (Row)com;
					
					if(RowUtils.isChecked(row))
					{
						OrderItem orderItem = orderItemService.getOne(RowUtils.id(row));
						if(orderItem != null)
						{
							WorkOrderItemFulfillment fulfillment = new WorkOrderItemFulfillment();
							fulfillment.setEffort(effort);
							fulfillment.setOrder(orderItem.getOrder().toRef());
							fulfillment.setOrderItem(orderItem.toRef());
							
							effort.getItemFulfillments().add(fulfillment);
						}
					}
				}
	...
	public void initForm()
	{
		...
		Row row0 = new Row();
		row0.appendChild(new Label(lang.get("workeffort.grid.column.organization")));
		row0.appendChild(organizations);
		row0.appendChild(new Label(lang.get("workeffort.grid.column.scheduledStart")));
		row0.appendChild(scheduledStart);

		Row row1 = new Row();
		row1.appendChild(new Label(lang.get("workeffort.grid.column.name")));
		row1.appendChild(name);
		row1.appendChild(new Label(lang.get("workeffort.grid.column.scheduledEnd")));
		row1.appendChild(scheduledEnd);
		
		Row row2 = new Row();
		row2.appendChild(new Label(lang.get("workeffort.grid.column.creationDate")));
		row2.appendChild(created);
		row2.appendChild(new Label(lang.get("workeffort.grid.column.actualStart")));
		row2.appendChild(actualStart);
		
		Row row3 = new Row();
		row3.appendChild(new Label(lang.get("workeffort.grid.column.type")));
		row3.appendChild(typeList);		
		row3.appendChild(new Label(lang.get("workeffort.grid.column.actualEnd")));
		row3.appendChild(actualEnd);
		
		Row row4 = new Row();
		row4.appendChild(new Label(lang.get("workeffort.grid.column.purpose")));
		row4.appendChild(purposeList);
		row4.appendChild(new Label(lang.get("workeffort.grid.column.maxhours")));
		row4.appendChild(maxhours);
		
		Row row5 = new Row();
		row5.appendChild(new Label());
		row5.appendChild(new Label());
		row5.appendChild(new Label(lang.get("workeffort.grid.column.maxmoney")));
		row5.appendChild(maxmoney);
	...
	private void initAssignments()
	{
		...
		assignments.getColumns().appendChild(new Column(lang.get("workeffort.grid.column.facility"),null,"185px"));
		assignments.getColumns().appendChild(new Column(lang.get("workeffort.grid.column.type"),null,"135px"));
	...
	private void initWorkRequirements()
	{
		...
		requirementFulfillments.getColumns().appendChild(new Column(null,null,"25px"));
		...
		requirementFulfillments.getColumns().appendChild(new Column());
		requirementFulfillments.setSpan("3");
		
		for(WorkRequirement requirement:workRequirementService.findAllForWorkEfforts())
		{
			Row row = new Row();
			row.appendChild(Components.checkbox(false));
	...
	private void initWorkOrders()
	{
		...
		orderFulfillments.getColumns().appendChild(new Column(null,null,"25px"));
		...
		orderFulfillments.getColumns().appendChild(new Column());
		orderFulfillments.getColumns().getLastChild().setVisible(false);
		orderFulfillments.setSpan("3");
		
		for(SalesOrderItem orderItem:salesOrderService.findAllForWorkEffort())
		{
			Row row = new Row();
			row.appendChild(Components.checkbox(false));
			...
			row.appendChild(new Label(Numbers.format(orderItem.getQuantity())));
			row.appendChild(new Label(orderItem.getNote()));
