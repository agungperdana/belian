...
{	
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				product = service.getOne(product.getId());
				if(product == null)
				{
					product = new Product();
				...
				Flow.next(getParent().getParent().getParent(), new ProductDashboard(service.getOne(product.getId())));
			}
		});
	}
	...
	{
		Product out = service.getOne(product.getId());
		if(out != null)
		{
