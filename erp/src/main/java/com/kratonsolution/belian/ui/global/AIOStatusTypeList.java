 ...
 */
public class AIOStatusTypeList extends AbstractList<AIOStatusType>
{
	public AIOStatusTypeList(boolean fullspan)
	{
		this(fullspan,null);
	}
	
	public AIOStatusTypeList(boolean fullspan,AIOStatusType con)
	{
		if(fullspan)
			setWidth("100%");
		else
		...
		for(AIOStatusType type:AIOStatusType.values())
		{
	...
	@Override
	public AIOStatusType getDomain()
	{
		return getSelectedItem()!=null?AIOStatusType.valueOf(getSelectedItem().getValue()):null;
	}

	@Override
	public IDValueRef getDomainAsRef()
	{
		AIOStatusType type = getDomain();
		if(type != null)
		{
	...
	@Override
	public void setDomain(AIOStatusType domain)
	{
		if(domain != null)
		{
			getItems().clear();
			...
			for(AIOStatusType type:maps.values())
			{
				Listitem listitem = appendItem(type.display(utils.getLanguage()), type.name());
