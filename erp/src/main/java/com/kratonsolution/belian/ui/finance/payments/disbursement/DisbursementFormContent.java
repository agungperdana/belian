...
import com.kratonsolution.belian.ui.finance.invoices.PurchaseInvoiceBox;
 ...
 */
public class DisbursementFormContent extends AbstractForm
{	
	...
	private PartyBox receiver = new PartyBox(false,false,false);
	...
	public DisbursementFormContent()
	{
		super();
		initToolbar();
		initForm();
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				if(receiver.getDomain() == null)
					throw new WrongValueException(receiver,lang.get("message.field.empty"));
				...
				disbursement.setAmount(amount.getValue());
				disbursement.setCommend(note.getText());
				disbursement.setEfectiveDate(DateTimes.sql(date.getValue()));
				disbursement.setMethod(typeList.getDomain());
				disbursement.setReceiver(receiver.getDomainAsRef());
				disbursement.setPayer(organizations.getDomainAsRef());
				disbursement.setReference(reference.getText());
				disbursement.setNumber(generator.generate(DateTimes.currentDate(), organizations.getDomain().getId(),Code.DMT));
				
				for(Component com:applications.getRows().getChildren())
				{
					Row row = (Row)com;
					
					PurchaseInvoiceBox box = (PurchaseInvoiceBox)row.getChildren().get(1);
	...
	private void initApplications()
	{
		...
		nrc.getNew().addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				PurchaseInvoiceBox box = new PurchaseInvoiceBox(false, true);
				box.addObserver(new Observer()
				{
					...
					public void valueChange(IDValueRef value)
					{
						...
						{
							Invoice invoice = invoiceService.getOne(value.getId());
							if(invoice != null)
							{
