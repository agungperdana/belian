 ...
 */
public class SalesOrderEditContent extends AbstractForm
{	
	...
	private PartyBox placingOrders = new PartyBox(false,false);
	
	private PartyBox billToParty = new PartyBox(false,false);

	private PartyAddressList billToAddress = new PartyAddressList(false, null);

	private PartyContactList billToContacts = new PartyContactList(false, null);

	private PartyBox shipToParty = new PartyBox(false,false);
	...
	public SalesOrderEditContent(Row row)
	{
		super();
		this.row = row;
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				if(billToContacts.getDomain() == null)
					throw new WrongValueException(billToContacts,lang.get("message.field.empty"));
				...
				if(order != null)
				{
					...
					order.setBillToParty(placingOrders.getDomainAsRef());
					order.setBillToAddress(billToAddress.getDomainAsRef());
					order.setBillToContact(billToContacts.getDomainAsRef());
					
					order.setPartyTakingOrder(organizations.getDomainAsRef());
					order.setPartyPlacingOrder(placingOrders.getDomainAsRef());
				...
				Flow.next(getParent(), new SalesOrderEditContent(row));
			}
		});
	}
	...
	public void initForm()
	{
		...
		if(order != null)
		{
			...
			placingOrders.setDomainAsRef(order.getPartyPlacingOrder());
		...
		row4.appendChild(placingOrders);
	...
	private void initBillings()
	{
		...
		if(order != null)
		{
			...
			billToAddress.setDomainAsRef(order.getBillToAddress());
			billToContacts.setDomainAsRef(order.getBillToContact());
		}
		...
		row3.appendChild(billToContacts);
	...
	private void initItems()
	{
		...
		if(order != null)
		{
			for(OrderItem item:order.getItems())
			{
				...
				row.addEventListener(Events.ON_CLICK, new EventListener<Event>()
				{
					...
					public void onEvent(Event event) throws Exception
					{
						Flow.next(getParent(), new SalesOrderItemViewForm(item));
					}
				});
		...
		nrc.getNew().addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				Flow.next(getParent(), new SalesOrderItemForm(salesOrderItem));
			}
		});
				...
				Messagebox.show(lang.get("message.removedata"),"Warning",Messagebox.CANCEL|Messagebox.OK, Messagebox.QUESTION,new EventListener<Event>()
				{
					...
					public void onEvent(Event event) throws Exception
					{
						if(event.getName().equals("onOK"))
						{
							...
							SalesOrder out = service.getOne(RowUtils.id(row));
							if(out != null)
							{
	...
	private void initRoles()
	{
		...
		nrc.getNew().addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				Row row = new Row();
				row.appendChild(Components.checkbox(false));
				row.appendChild(new PartyBox(false, true));
