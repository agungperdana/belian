...
public class ShipmentIssuanceService extends AbstractService
{
	...
	@Secured("ROLE_SHIPMENT_ISSUANCE_READ")
	public ShipmentIssuance getOne(String id)
	{
	...
	public void add(ShipmentIssuance issuance)
	{
		...
		{
			ShipmentItem shipmentItem = itemRepo.getOne(item.getShipmentItem().getId());
		...
		Shipment shipment = shipmentRepo.getOne(issuance.getShipment().getId());
		if(shipment != null)
		{
	...
	{
		ShipmentIssuance issuance = getOne(id);
		if(issuance != null)
		{
