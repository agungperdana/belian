...
public class StockService extends AbstractService
{
	...
	{
		Product product = productRepository.getOne(prodRef.getId());
		...
		if(!product.getType().equals(ProductType.SERVICE) && product.getComponents().isEmpty())
		{
			...
			if(expired != null)
				inventoryItem = repository.getOne(product.getId(), facility.getId(), container.getId(), expired);
			else
				inventoryItem = repository.getOne(product.getId(), facility.getId(), container.getId());
