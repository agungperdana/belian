...
public class InventoryItemService
{
	...
	@Secured("ROLE_INVITEM_READ")
	public InventoryItem getOne(String id)
	{
		return repository.getOne(id);
	}
	
	@Secured("ROLE_INVITEM_READ")
	public InventoryItem getOne(String product,String facility)
	{
		return repository.getOne(product,facility);
	}
	
	@Secured("ROLE_INVITEM_READ")
	public InventoryItem getOne(String product,String facility,Date expired)
	{
		if(expired == null)
			getOne(product, facility);
	
		return repository.getOne(product, facility, expired);
	}
	...
	{
		InventoryItem ondb = repository.getOne(item.getProduct().getId(),item.getFacility().getId(),item.getContainer().getId(),item.getExpiredDate());
		if(ondb != null)
