...
public class PurchaseInvoiceService extends AbstractService
{
	...
	@Secured("ROLE_PURCHASE_INVOICE_READ")
	public PurchaseInvoice getOne(String id)
	{
	...
	public void delete(Collection<String> ids)
	{
		for(String id:ids)
		{
			PurchaseInvoice invoice = getOne(id);
	...
	public void delItem(Collection<String> ids)
	{
		...
		{
			PurchaseInvoice invoice = repository.getOne(ids);
			if(invoice != null)
			{
	...
	{
		PurchaseInvoice invoice = repository.getOne(id);
		if(invoice != null && !invoice.isPaid())
		{
			BigDecimal amt = BigDecimal.ZERO;
	...
	public Invoice fromPurchaseOrder(PurchaseOrder order)
	{
		...
		//payer
		Party payer = partyRepo.getOne(order.getPartyPlacingOrder().getId());
		if(payer != null)
		{
		...
		Party receiver = partyRepo.getOne(order.getPartyTakingOrder().getId());
		if(receiver != null)
		{
