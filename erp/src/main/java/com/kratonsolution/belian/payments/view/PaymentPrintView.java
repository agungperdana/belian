...
public class PaymentPrintView extends AbstractView
{
	...
	{
		Receipt receipt = service.getOne(id);
		...
		{
			Invoice invoice = invRepo.getOne(application.getInvoice().getId());
			if(invoice != null)
			{
				for(InvoiceItem item:invoice.getItems())
				{
					...
					{
						OrderItem orderItem = itemRepo.getOne(billing.getOrderItem().getId());
						if(orderItem != null)
						{
		...
		model.addAttribute("receiver",partyService.getOne(receipt.getReceiver().getId()));
		model.addAttribute("payer",partyService.getOne(receipt.getPayer().getId()));
