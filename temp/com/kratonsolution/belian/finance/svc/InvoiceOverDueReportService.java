...
public class InvoiceOverDueReportService
{	
	@Autowired
	...
	@Secured({"ROLE_INVOICE_OVERDUE_REPORT_READ"})
	public Map<String,Object> generate(String customer,String company)
	{
		...
		map.put("title",lang.get("invoiceoverdue.label.title"));
		...
		for(String com:structureIds.values())
		{
			...
			if(Strings.isNullOrEmpty(customer))
				incomes.addAll(billableRepository.findAllOverDueUnpaid(com,DateTimes.currentDate()));
			else
				incomes.addAll(billableRepository.findAllOverDueUnpaid(com,customer,DateTimes.currentDate()));
