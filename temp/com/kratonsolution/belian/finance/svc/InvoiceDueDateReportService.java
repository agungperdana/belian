...
public class InvoiceDueDateReportService
{	
	@Autowired
	...
	@Secured({"ROLE_INVOICE_DUEDATE_REPORT_READ"})
	public Map<String,Object> generate(String customer,String company)
	{
		...
		map.put("title",lang.get("invoiceduedate.label.title"));
		...
		for(String com:structureIds.values())
		{
			...
			if(Strings.isNullOrEmpty(customer))
				incomes.addAll(billableRepository.findAllDueUnpaid(com,DateTimes.currentDate()));
			else
				incomes.addAll(billableRepository.findAllDueUnpaid(com,customer,DateTimes.currentDate()));
