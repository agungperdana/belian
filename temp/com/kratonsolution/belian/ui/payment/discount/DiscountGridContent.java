 ...
 */
public class DiscountGridContent extends GridContent
{
	private DiscountService service = Springs.get(DiscountService.class);
	
	public DiscountGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new DiscountModel(utils.getRowPerPage()));
			}
		});
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				DiscountWindow window = (DiscountWindow)getParent();
				window.removeGrid();
	...
	{
		final DiscountModel model = new DiscountModel(utils.getRowPerPage());
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage(lang.get("message.grid.empty"));
		grid.setModel(model);
		grid.setRowRenderer(new DiscountRowRenderer());
		...
		grid.getColumns().appendChild(new Column(lang.get("generic.grid.column.amount"),null,"90px"));
