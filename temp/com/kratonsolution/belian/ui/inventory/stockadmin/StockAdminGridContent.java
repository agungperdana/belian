...
public class StockAdminGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new StockAdminModel(utils.getRowPerPage()));
			}
		});
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				StockAdminWindow window = (StockAdminWindow)getParent();
				window.removeGrid();
	...
	{
		final StockAdminModel model = new StockAdminModel(utils.getRowPerPage());
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage("No product stock administrator data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new StockAdminRowRenderer());
		...
		grid.getColumns().appendChild(new Column("Company",null,null));
