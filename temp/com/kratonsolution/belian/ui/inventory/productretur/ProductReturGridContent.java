 ...
 */
public class ProductReturGridContent extends GridContent
{
	private ProductReturService service = Springs.get(ProductReturService.class);
	
	private SessionUtils utils = Springs.get(SessionUtils.class);
	
	public ProductReturGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new ProductReturModel(utils.getRowPerPage()));
			}
		});
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				ProductReturWindow window = (ProductReturWindow)getParent();
				window.removeGrid();
	...
	{
		final ProductReturModel model = new ProductReturModel(utils.getRowPerPage());
		
		grid.setHeight("80%");
		grid.setEmptyMessage("No product retur data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new ProductReturRowRenderer());
		...
		grid.getColumns().appendChild(new Column("Supplier",null,"125px"));
