 ...
 */
public class TransferOrderRequestEditContent extends FormContent
{	
	...
	public TransferOrderRequestEditContent(Row row)
	{
		super();
		this.row = row;
		initToolbar();
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				TransferOrderRequest request = service.findOne(RowUtils.string(row, 5));
				if(request != null && request.getRequestStatus().equals(RequestStatus.INCOMPLETE) && request.getApproverStatus().equals(ApproverStatus.SUBMITTED))
				{
					...
					for(Component com:items.getRows().getChildren())
					{
						...
						TransferOrderRequestItem item = new TransferOrderRequestItem();
	...
	public void initForm()
	{
		...
		if(request != null)
		{
			if(Strings.isNullOrEmpty(request.getNumber()))
				request.setNumber(TransferOrderRequest.NCODE+System.currentTimeMillis());
	...
	public void initItems()
	{
		...
		if(request != null)
		{
			...
			items.getColumns().appendChild(new Column("Fullfilled",null,"100px"));
			...
			for(TransferOrderRequestItem item:request.getItems())
			{
				...
				row.appendChild(Components.label(item.getFullfilled()));
