 ...
 */
public class GoodsTransferGridContent extends GridContent
{
	private GoodsTransferService service = Springs.get(GoodsTransferService.class);
	
	private SessionUtils utils = Springs.get(SessionUtils.class);
	
	public GoodsTransferGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new GoodsTransferModel(utils.getRowPerPage()));
			}
		});
	...
	{
		final GoodsTransferModel model = new GoodsTransferModel(utils.getRowPerPage());
		
		grid.setHeight("80%");
		grid.setEmptyMessage("No goods transfer data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new GoodsTransferRowRenderer());
		...
		grid.getColumns().appendChild(new Column("Company From",null,"150px"));
		grid.getColumns().appendChild(new Column("Facility From",null,"125px"));
		grid.getColumns().appendChild(new Column("Company To",null,"150px"));
		grid.getColumns().appendChild(new Column("Facility To",null,"125px"));
		grid.getColumns().appendChild(new Column("Request No",null,"125px"));
