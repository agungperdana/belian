...
public class StudentGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				StudentWindow window = (StudentWindow)getParent();
				window.removeGrid();
	...
	protected void initGrid()
	{
		...
		filter.addEventListener(Events.ON_CHANGING,new EventListener<InputEvent>()
		{
			...
			{
				refresh(new StudentModel(utils.getRowPerPage(), input.getValue()));
			}
		});
		...
		final StudentModel model = new StudentModel(utils.getRowPerPage());
		
		grid.setHeight("80%");
		grid.setEmptyMessage(lang.get("message.grid.empty"));
		grid.setModel(model);
		grid.setRowRenderer(new StudentRowRenderer());
