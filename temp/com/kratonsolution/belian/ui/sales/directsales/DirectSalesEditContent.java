 ...
 */
public class DirectSalesEditContent extends FormContent implements ProductPriceSelectionListener
{	
		...
		if(cash != null)
		{
			...
			toolbar.getChildren().get(0).addEventListener(Events.ON_CLICK,new EventListener<Event>()
			{
				...
				public void onEvent(Event event) throws Exception
				{
					...
					row.addProductPriceListener(DirectSalesEditContent.this);
					saleItems.getRows().appendChild(row);
				}
			});
