 ...
 */
public class SalesInvoiceReportForm extends AbstractForm
{
	private Datebox from = Components.currentDatebox();
	
	private Datebox to = Components.currentDatebox();
	
	private Checkbox paid = Components.checkbox(false);
	
	private PartyBox customers = new PartyBox(false);
	...
	public void initToolbar()
	{
		...
		toolbar.getSave().addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				builder.append("/salesinvoicereportview?organization="+organizations.getDomain());
				builder.append("&from="+DateTimes.format(from.getValue()));
				builder.append("&to="+DateTimes.format(to.getValue()));
				builder.append("&paid="+paid.isChecked());
				builder.append("&customer="+customers.getDomainAsRef().getId());
	...
	private void initRow()
	{
		...
		Row row2 = new Row();
		row2.appendChild(new Label(lang.get("generic.grid.column.organization")));
		row2.appendChild(customers);
		
		Row row3 = new Row();
		row3.appendChild(new Label(lang.get("generic.grid.column.fromdate")));
		row3.appendChild(from);
		
		Row row4 = new Row();
		row4.appendChild(new Label(lang.get("generic.grid.column.todate")));
		row4.appendChild(to);
		
		Row row5 = new Row();
		row5.appendChild(new Label(lang.get("invoices.grid.column.includepaid")));
		row5.appendChild(paid);
