 ...
 */
public class StudyRoomGridContent extends GridContent
{
	private StudyRoomService service = Springs.get(StudyRoomService.class);
	
	public StudyRoomGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new StudyRoomModel(utils.getRowPerPage()));
			}
		});
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new StudyRoomFormContent());
			}
		});
	...
	{
		filter.setPlaceholder(lang.get("message.field.iden"));
		filter.addEventListener(Events.ON_CHANGING, new EventListener<InputEvent>()
		{
			...
			{
				refresh(new StudyRoomModel(utils.getRowPerPage(),input.getValue()));
			}
		});
		...
		final StudyRoomModel model = new StudyRoomModel(utils.getRowPerPage());
		
		grid.setHeight("80%");
		grid.setEmptyMessage(lang.get("message.grid.empty"));
		grid.setModel(model);
		grid.setRowRenderer(new StudyRoomRowRenderer());
		grid.setPagingPosition("both");
		grid.setMold("paging");
		grid.setPageSize(utils.getRowPerPage());
		grid.appendChild(new Columns());
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.name"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.room"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.course"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.period"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.day"),null,"125px"));
		grid.getColumns().appendChild(new Column(lang.get("studyroom.grid.column.time"),null,"125px"));
