 ...
 */
public class SendGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				grid.setModel(new SendModel(utils.getRowPerPage()));
			}
		});
	...
	{
		final SendModel model = new SendModel(utils.getRowPerPage());
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage("No Send message data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new SendRowRenderer());
