 ...
 */
public class DeductionTypeModel implements ListModel<DeductionType>
{
	private final DeductionTypeService service = Springs.get(DeductionTypeService.class);
	
	private List<DeductionType> data = new ArrayList<DeductionType>();
	
	public DeductionTypeModel(int itemSize)
	{
		next(0, itemSize);
	}
	
	@Override
	public DeductionType getElementAt(int index)
	{
		if(index >= data.size())
			return null;
