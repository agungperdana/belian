 ...
 *
 */
public class CashAccountGridContent extends GridContent
{
	private final CashAccountService service = Springs.get(CashAccountService.class);
	
	public CashAccountGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				grid.setModel(new CashAccountModel(8));
			}
		});
	...
	{
		final CashAccountModel model = new CashAccountModel(8);
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage("No Cash Account data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new CashAccountRowRenderer());
		...
		Column currency = new Column("Currency",null,"75px");
		Column owner = new Column("Owner",null,"150px");
