 ...
 *
 */
public class CashAccountModel implements ListModel<CashAccount>
{
	private final CashAccountService service = Springs.get(CashAccountService.class);
	
	private List<CashAccount> data = new ArrayList<CashAccount>();
	
	public CashAccountModel(int itemSize)
	{
		next(0, itemSize);
	}
	
	@Override
	public CashAccount getElementAt(int index)
	{
		if(index >= data.size())
			return null;
