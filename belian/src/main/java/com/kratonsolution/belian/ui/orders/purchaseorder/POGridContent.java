...
public class POGridContent extends GridContent
{
	...
	protected void initToolbar()
	{
		...
		toolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new POFormContent());
			}
		});
	...
	protected void initGrid()
	{
		...
		filter.addEventListener(Events.ON_CHANGING, new EventListener<InputEvent>()
		{
			...
			{
				grid.setModel(new POModel(utils.getRowPerPage(), input.getValue()));
			}
		});

		final POModel model = new POModel(utils.getRowPerPage(),filter.getText());

		grid.setHeight("80%");
		grid.setEmptyMessage(lang.get("message.grid.empty"));
		grid.setModel(model);
		grid.setRowRenderer(new PORowRenderer());
		...
		grid.getColumns().appendChild(new Column(lang.get("order.grid.column.number"),null,"100px"));
		grid.getColumns().appendChild(new Column(lang.get("order.grid.column.placingorder"),null,"250px"));
		grid.getColumns().appendChild(new Column(lang.get("order.grid.column.takingorder"),null,"250px"));
