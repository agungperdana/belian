 ...
 */
public class InvoiceDueDateReportFormContent extends ReportForm
{	
	private PersonBox persons = new PersonBox(false);
	
	private OrganizationList companys = new OrganizationList();
		
	public InvoiceDueDateReportFormContent()
	{
		super();
		initToolbar();
		initForm();
	...
	public void initToolbar()
	{
		...
		toolbar.getGenerate().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				Flow.next(getParent(), new InvoiceDueDateReportResultContent(persons.getNullablePersonId(), companys.getOrganization().getId()));
			}
		});
	}
