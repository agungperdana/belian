 ...
 */
public class AssetTypeGridContent extends GridContent
{
	private FacilityService service = Springs.get(FacilityService.class);
	
	private SessionUtils utils = Springs.get(SessionUtils.class);
	
	public AssetTypeGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				refresh(new AssetTypeModel(utils.getRowPerPage()));
			}
		});
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				AssetTypeWindow window = (AssetTypeWindow)getParent();
				window.removeGrid();
	...
	{
		final AssetTypeModel model = new AssetTypeModel(utils.getRowPerPage());
		
		grid.setHeight("80%");
		grid.setEmptyMessage("No facility data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new AssetTypeRowRenderer());
