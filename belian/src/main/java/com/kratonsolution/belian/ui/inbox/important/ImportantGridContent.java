 ...
 */
public class ImportantGridContent extends GridContent
{
	private SessionUtils utils = Springs.get(SessionUtils.class);
	
	private ImportantService service = Springs.get(ImportantService.class);
	
	public ImportantGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getRefresh().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				...
				grid.setModel(new ImportantModel(utils.getRowPerPage()));
			}
		});
	...
	{
		final ImportantModel model = new ImportantModel(utils.getRowPerPage());
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage("No inbox(important) message data exist.");
		grid.setModel(model);
		grid.setRowRenderer(new ImportantRowRenderer());
