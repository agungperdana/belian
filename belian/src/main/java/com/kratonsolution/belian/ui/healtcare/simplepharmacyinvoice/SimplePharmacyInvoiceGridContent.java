 ...
 */
public class SimplePharmacyInvoiceGridContent extends GridContent
{
	private SimplePharmacyInvoiceService service = Springs.get(SimplePharmacyInvoiceService.class);
	
	public SimplePharmacyInvoiceGridContent()
	{
		super();
		initToolbar();
		initGrid();
	...
	protected void initToolbar()
	{
		...
		gridToolbar.getNew().addEventListener(Events.ON_CLICK,new EventListener<Event>()
		{
			...
			public void onEvent(Event event) throws Exception
			{
				Flow.next(getParent(), new SimplePharmacyInvoiceFormContent());
			}
		});
				...
				Messagebox.show(lang.get("message.removedata"),"Warning",Messagebox.CANCEL|Messagebox.OK, Messagebox.QUESTION,new EventListener<Event>()
				{
					...
					public void onEvent(Event event) throws Exception
					{
						if(event.getName().equals("onOK"))
						{
							...
							refresh(new SimplePharmacyInvoiceModel(utils.getRowPerPage()));
						}
					}
				});
			}
	...
	{
		final SimplePharmacyInvoiceModel model = new SimplePharmacyInvoiceModel(utils.getRowPerPage());
		
		grid.setParent(this);
		grid.setHeight("80%");
		grid.setEmptyMessage(lang.get("message.grid.empty"));
		grid.setModel(model);
		grid.setRowRenderer(new SimplePharmacyInvoiceRowRenderer());
		grid.setPagingPosition("both");
		grid.setMold("paging");
		grid.setPageSize(utils.getRowPerPage());
		grid.appendChild(new Columns());
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column(lang.get("simplepharmacyinvoice.grid.column.date"),null,"85px"));
		grid.getColumns().appendChild(new Column(lang.get("simplepharmacyinvoice.grid.column.customer"),null,"150px"));
		grid.getColumns().appendChild(new Column(lang.get("simplepharmacyinvoice.grid.column.amount"),null,"150px"));
		grid.getColumns().appendChild(new Column(lang.get("simplepharmacyinvoice.grid.column.note"),null,"85px"));
