...
{
	@Query("FROM DoctorAppointment app WHERE app.date =:date AND app.company.id =:company AND app.doctor.from.id =:doctor AND (app.status != 'CANCELED' AND app.status != 'DONE') ORDER BY app.queue ASC")
	...
	@Query("FROM DoctorAppointment app WHERE app.date =:date AND app.patient.from.name LIKE :customer% AND app.company.id =:company AND app.doctor.from.id =:doctor AND (app.status != 'CANCELED' AND app.status != 'DONE') ORDER BY app.queue ASC")
